...
async def create_user(request: HttpRequest, data: BotUserCreateSchema):
    try:
        ...
        return responses.JsonResponse(BotUserOutSchema.from_orm(user).dict(), status=HTTPStatus.CREATED)
...
@router_user.get('/users/{telegram_id}',
                 response={HTTPStatus.OK: BotUserOutSchema,
                           HTTPStatus.BAD_REQUEST: ErrorSchema},
                 summary='Получение пользователя по ID')
async def get_user(request: HttpRequest, telegram_id: int):
    try:
        user = await bot_user_service.get_bot_user(telegram_id)
        return responses.JsonResponse(BotUserOutSchema.from_orm(user).dict(), status=HTTPStatus.OK)
    except UserNotFoundError as e:
        return responses.JsonResponse({'detail': str(e)}, status=HTTPStatus.NOT_FOUND)


@router_user.get('/users/{telegram_id}/boxes',
                 response={HTTPStatus.OK: list[EmailBoxOutputSchema],
